...
from arteris_app.api.measurement import get_measurement
...
class ContractMeasurementNote(Document):
    ...
    def before_submit(self):
        ...
        if not measurement:
            frappe.throw("Falha ao obter ou gerar o boletim de medição, verifique configurações do contrato!")
            return False
        ...
        for highway in get_note.tabrodovias:
            ...
            cities = frappe.db.sql("""
                ...
                FROM
                    `tabHighway` h
                    LEFT JOIN `tabHighway City` hc ON h.name = hc.parent AND hc.kminicial BETWEEN %(kminicial)s AND %(kmfinal)s
                WHERE
                    h.name = %(rodovia)s
            """, {"rodovia": highway.rodovia, "kminicial": highway.kminicial, "kmfinal": highway.kmfinal}, as_dict=True)

            # Cria um registro por cidade
            for c in cities:
                measurement_highway_record = measurement_record.append("tablog")
                measurement_highway_record.dataexecucao = current_date
                measurement_highway_record.cidade_name = c.cidade_name
                measurement_highway_record.rodovia_name = highway.rodovia
                measurement_highway_record.rodovia = highway.rodovia
                measurement_highway_record.kminicial = highway.kminicial
                measurement_highway_record.kmfinal = highway.kmfinal
    ...
    def before_save(self):
        ...
        if (self.datasubmissao):
            return True
        
        if (not flt(self.totalmedido, 2) == flt(self.totalsap, 2)):
            frappe.throw("A soma dos valores medidos não é igual a soma dos valores das linhas de pedido SAP.")
            return False
        return True
...
def get_asset_item(doctype, txt, searchfield, start, page_len, filters):
    ...
    result = frappe.db.sql("""
        ...
        WHERE 
            ...
            AND (item.name LIKE %(txt)s OR item.codigo LIKE %(txt)s OR item.descricao LIKE %(txt)s)
        ORDER BY 
             INET_ATON(SUBSTRING_INDEX(CONCAT(item.codigo,'.0.0.0.0.0.0.0.0'), '.', 8)), item.descricao
        LIMIT %(start)s, %(page_len)s""", 
        {
            'contrato': contrato,
            'ativo': ativo,  

...
def write_no_records():
    ...
    integration_inconsistency.tipo = "Osiris"
...
def upload_image():
    try:
        ...
        production_id = frappe.form_dict.production_id
        ...
        upload_response = upload_file(doctype, production_id, file_64, file_name)

        record_name = frappe.db.get_list(doctype, {"productionid": production_id}, "name")

        if not record_name or len(record_name) == 0:
            return {"success": False, "message": f"No {doctype} found with production ID {production_id}"}
        
        for r in record_name:
...
def get_keys(contract_name: str, contract_processing_date: str):
        ...
        if get_result:
            for l in get_result:
                # Get the Integration Record Keys for the Integration Record
                get_result = frappe.db.get_all("Integration Record Keys", fields=["uuid"], filters={"parent": l.name})
                if get_result:
                    # If keys found, extend the keys list
                    keys.extend([k.uuid for k in get_result])
...
def create_osiris_measurement_record(
    ...
    for d in data:
        ...
        if not osiris_measurement_record:
            ...
            osiris_measurement_record.productionid = d["productionid"]
        ...
        if has_inconsistent_data:
            ...
            if d["chave_recurso"]:
                ...
                if not d["id"] in osiris_uuids:
                    ...
                    osiris_measurement_reseource_record.quantidademedida = d["quantity"]
...
def get_list_log_to_attach_images():
    records = frappe.db.sql("""
        ...
        WHERE
            ...
            NOT cmr.productionid IS NULL AND 
            cmr.datahoraimagens IS NULL AND 
            cmr.origem_integracao LIKE '%Osiris%'
        ORDER BY
            cmr.name,
            cmr.productionid
    """, as_dict=True)
    ...
    for record in records:
        get_images.setdefault(record.productionid, {"uuids": []})
        get_images[record.productionid]["uuids"].append(record.measurement_record)

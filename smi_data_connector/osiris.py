...
def get_prod_with_session(
    ...
    try:
        ...
        raise Exception(f"Erro ao buscar produções com sessão: {e}, URL: {producoes_url},  Params: {params}")
    
def get_images_with_session(
        session, 
        page_number=1, 
        page_size=10, 
        production_id=None,
        base_url = "https://preprod-osiris.arteris.com.br/api/api") -> Dict[str, Any]:
    """
    Versão de get_image que utiliza uma sessão autenticada

    Parâmetros:
        session: Sessão autenticada para requisições
        page_number: Número da página para paginação (padrão: 1)
        page_size: Tamanho da página para paginação (padrão: 10)
        production_id: ID da produção para filtrar imagens (opcional)
        base_url: URL base da API OSIRIS (padrão: ambiente de pré-produção)

    Retorno:
        Dados JSON com informações das produções ou erro em caso de falha
    """
    images_url = f"{base_url}/mediaproduction/{production_id}"
    params = {
        "PageNumber": page_number,
        "PageSize": page_size
    }
    ...
    try:
        response = session.get(images_url, params=params)
        response.raise_for_status()  # Lança um erro para códigos de status HTTP 4xx/5xx
        print(f"Response Status Code: {response.status_code}, URL: {images_url}, Params: {params}")
        return response.json()
    except requests.RequestException as e:
...
def create_osiris_measurement_records(
    ...
    def load_osiris_data():
        ...
        if ignore_check:
            ...
            for cp, values in contracts_name_to_process.items():
                if not values['uuid']:
                    print(f"Contrato {cp} sem UUID OSIRIS, ignorando verificação de dados")
        ...
        while search_date <= end_date:
            ...
            for cp in contracts_with_data:
                ...
                logger.info(f"Processando contrato: {cp} Contador: {counter_01}")
                ...
                while page > 0:
                    ...
                    print(f"Consultando produções - contrato: {cp}, data: {search_date.strftime('%Y-%m-%d')}, página: {page}")
                    productions = get_prod_with_session(
                        session, 
                        page_number = page, 
                        page_size = 5000,
                    ...
                    print(f"Produções retornadas: {len(productions['Data'])}")

                    for p in productions['Data']:

                        counter_02 += 1

                        print(f"MeasurementInfo: {p['MeasurementInfo']}")

                        # Ignorar se for nulo
                        if p['MeasurementInfo']:

                            print(f"Produção possui {len(p['MeasurementInfo'])} composições")
                            ...
                            for m in p['MeasurementInfo']:
                                ...
                                print(f"Processando composição {mcount+1} de {len(p['MeasurementInfo'])} - Medido: {m['IsMeasured']}")
                                ...
                                print(f"Composição é medida, processando...")
                                ...
                                if contract["process"]:                
                                    ...
                                    id = f"{p['ProductionId']}{p['Rdo']['Code']}{p['ServiceOrder']['Code']}{p['DateProduction'][:10]}{search_date.strftime('%Y%m%d')}{p['Id']}"
                                    key = id

                                    print(f"Processando registro: {key}")

                                    # Adicionar o registro aos registros de medição
                                    if id in contract["keys"]:
                                        print(f"Registro {key} já processado, ignorando...")
                                        continue                                
                                    ...
                                    processes_productions_images.setdefault(p['ProductionId'], False)
        ...
        return processes_productions_images


    def load_osiris_images(process_productions_images):
        ...
        for key, values in process_productions_images.items():
        
            # Consultar dados na API
            images = get_images_with_session(
                session, 
                page_number = 1, 
                page_size = 500,
                production_id = key)        
            ...
            if images:
                ...
                for image in images:
                    ...
                    # Enviar imagem para o Osiris
                    _url = f"{API_BASE_URL}/method/arteris_app.api.osiris.upload_image"
                    ...
                    filename = f'{key}-FILENAME.{compressed_image["format"].lower()}'
    ...
    process_productions_images = load_osiris_data()

    if not ignore_images:
        load_osiris_images(process_productions_images)
